...
function Texture::evalOperationData(%data, %row)
{
   ...
   // Simple hack to make sure the water mask gets updated
   if (%label $= "Place by Water Level" && $worldWaterDirty == true)
   {
      $dirtyTexture[%reg] = true;
      $worldWaterDirty = false;
   }
...
$TerraformerHeightfieldDir = "creator/terrain_imports";
...
function Heightfield::restoreTab(%id)
{
   ...
   // Hack to make sure the "meters" value is initially updated
   %label = getField(%data,0);
   
   if (%label $= "General")
      eval(general_water.command);
