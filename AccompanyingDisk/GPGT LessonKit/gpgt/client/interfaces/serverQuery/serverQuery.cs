...
// Scripts
//--------------------------------------------------------------------------
function serverQuery::onWake( %this )
{
   currentServerInfo.setValue("");
   serverQueryStatus.setValue("");
}

function serverQueryTextList::onWake( %this ) 
{
	lanQueryTextListSelect.performClick();
}

function serverQueryTextList::onSelect( %this , %selID )
{
   currentServerInfo.updateInfo( %selID );
}

function currentServerInfo::updateInfo( %this, %index )
{
   currentServerInfo.setValue("" );
   setServerInfo( %index );
...
function serverQueryTextList::refreshCurrentSelection( %this ) 
{
   querySingleServer( $ServerInfo::Address, 0 );
}

function serverQueryTextList::updateList( %this ) 
{
	// Let's start fresh
	%this.clear();
   ...
   {
      setServerInfo( %count );
      %this.addRow( %count, $ServerInfo::Name  );
   }
...
function serverQueryTextList::startQuery( %this , %queryType ) 
{
