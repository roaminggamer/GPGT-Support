...
function toggleClouds( %val )
{
   if( !%val ) return;
   
   commandToServer('toggleClouds');
}

// 1 - Create and define the new action map
new ActionMap(lessonActionMap);
lessonActionMap.bind(keyboard, "1", "toggleClouds");
...
LessonMessageText.addText( "Clouds fade in/out over a 1.5 second period when you press a key.<br><br>" , true ); // Add messages
LessonMessageText.addText( "Press '1' to toggle cloud fading.<br><br>" , true ); // Add messages
