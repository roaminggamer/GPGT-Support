...
// Scripts
//--------------------------------------------------------------------------
function TestTextList::onWake( %this ) {
...
function TestTextList::onSelect( %this , %selID )
{
   %rowText = %this.getRowTextByID(%selID);
   targetImage.setBitmap( expandFileName( %this.profile.productImage[%rowText] ) );
}


function TestTextList::addMyTextEntries( %this , %profile ) {
	...
	%tmptextEntrys = %this.profile.textEntryEntries;

	while( "" !$= %tmptextEntrys ) {
		%tmptextEntrys = nextToken( %tmptextEntrys , "textEntryToken" , ""TAB"");

		%this.addRow( %textEntryCount++, %textEntryToken  );
     
	}

	%this.sort( 0 , true ); // Sort in ascending alphabetical order

   %row = %this.findTextIndex("Game Programmer's Guide to Torque");
...
function TestTextList::updateTextList( %this , %profileNum ) {
...
//
function OpenWebButton::openThePage( %this ) {
