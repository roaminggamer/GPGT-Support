...
// Scripts
//--------------------------------------------------------------------------
function TestPopUpMenus::onWake( %this ) {
	//
	// Always start on the Play Games Menu
	//
	%this.addMyMenus( gsPopUpMenuPlayGames0 , "Play Games!");
}
...
function TestPopUpMenus::addMyMenus( %this , %profile , %Message) {
	...
	%tmpMenus = %this.profile.menuEntries;

	while( "" !$= %tmpMenus ) {
		%tmpMenus = nextToken( %tmpMenus , "menuToken" , ""TAB"");

		%this.add( %menuToken , %menuCount++ );
	}
...
function TestPopupMenuGroup::updateMenus( %this , %profileNum ) {
	...
	%this.getObject( 0 ).addMyMenus( %profileName , %message );
	%this.getObject( 1 ).addMyMenus( %profileName , %message );
...
//
function TestPopUpMenus::onSelect( %this , %entryID , %entryText) {
	if( "" $= %this.profile.links[%entryText] ) return;

	gotoWebPage( %this.profile.links[%entryText] );
}
