...
function TestMenuBar::onWake( %this ) {
	//
	// Always start on the Play Games Menu
	//
	%this.addMyMenus(gsPlayGamesProfile);
}
...
function TestMenuBar::addMyMenus( %this , %profile ) {
	...
	%tmpMenus = %this.profile.menus;

	while( "" !$= %tmpMenus ) {
		%tmpMenus = nextToken( %tmpMenus , "menuToken" , ""TAB""); 

		%this.addMenu( %menuToken , %menuCount++ );

		%tmpSubMenu = %this.profile.subMenu[%menuToken];
		...
		while( "" !$= %tmpSubMenu ) {
			...
			%this.addMenuItem( %menuToken , %subMenuToken , %subMenuCount++);

			// If the sub-menu is marked as inactive, disable it
			if(%this.profile.inactiveSubMenus[%subMenuToken]) {
				%this.setMenuItemEnable( %menuToken , %subMenuToken , false );
			}
		}
	}
}
...
function TestMenuBar::onMenuItemSelect( %this , %menuID , %menuName , %menuItemID , %menuItemName ) {
	if( "" !$= %this.profile.links[%menuItemName] )
		gotoWebPage( %this.profile.links[%menuItemName] );
}
